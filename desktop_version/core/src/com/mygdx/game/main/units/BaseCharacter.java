...
public abstract class BaseCharacter implements CharInterface, Comparable<BaseCharacter> {
    ...
    public int attackRange;              static private int defaultAttackRange = 1;
    protected int initiative;               static private int defaultInitiative = 50;
    protected boolean gotBuff;

    protected BaseCharacter
    (String name, int maxHealthPointsIncrese, int defenseIncrease, int speedIncrease,
     int attackIncrease, int damageDispersionIncrease, int attackRangeIncrese, int initiativeIncrease) {
        this.name = name;
        image = new Texture("images/" + name.substring(0, name.length()-2) + ".png");
        alive = true;
